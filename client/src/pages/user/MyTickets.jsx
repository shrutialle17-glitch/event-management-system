...
const MyTickets = () => {
  ...
  const downloadCertificate = async (registrationId, ticketId) => {
    try {
      const response = await axiosInstance.get(
        `/attendance/${registrationId}/certificate`,
        {
          responseType: "blob",
        },
      );
      ...
      toast.error(
        err.response?.data?.message || "Certificate not available yet.",
      );
    }
  };
      ...
      {loading ? (
        ...
        <div className="space-y-8">
          {displayList.map((reg) => (
            <div
              key={reg._id}
              className="bg-white rounded-2xl border border-gray-100 shadow-sm overflow-hidden flex flex-col lg:flex-row"
            >
              {/* Event Info Side */}
                  ...
                  <div className="flex justify-between items-start mb-4">
                    ...
                    {reg.status === "waitlisted" && (
                      <span className="bg-amber-100 text-amber-700 px-3 py-1 rounded-full text-xs font-bold uppercase">
                        Waitlisted — Position #{reg.waitlistPosition}
                      </span>
                    )}
                ...
                <div className="border-t border-gray-100 pt-6 mt-auto">
                  ...
                  {activeTab === "past" && reg.status === "checked-in" && (
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                      ...
                      {reg.hasFeedback ? (
                        <div className="bg-green-50 border border-green-200 rounded-xl p-6 flex items-center justify-center">
                  ...
                  {activeTab === "past" && reg.status !== "checked-in" && (
                    <p className="text-amber-600 text-sm italic">
                      You did not check in to this event.
                    </p>
                  )}
                </div>
              </div>
                ...
                <div className="lg:w-80 bg-gray-50 border-t lg:border-t-0 lg:border-l border-dashed border-gray-300 p-8 flex flex-col items-center justify-center relative">
                  ...
                  {reg.status === "waitlisted" ? (
                    <>
                      ...
                      <p className="text-xs text-center text-textMuted px-4 leading-relaxed">
                        You will receive an email and QR code if a spot opens
                        up.
                      </p>
                    </>
                  ) : (
                    <>
                      <p className="text-sm font-bold text-textMuted uppercase tracking-widest mb-6">
                        Admit One
                      </p>
                      ...
                      {reg.status === "checked-in" && (
                        <Button
                          ...
                          onClick={() =>
                            downloadCertificate(reg._id, reg.ticketId)
                          }
                        >
